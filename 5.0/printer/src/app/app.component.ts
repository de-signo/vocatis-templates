    ...
    timer(0, 5 * 60 * 1000).pipe(
      switchMap(_ => {
        ...
        if (activeStyle == "groups") {
          ...
          return this.dataService.loadAppointments().pipe(
            concatMap(_ => this.dataService.loadButtons()));
        }
        else
          return of(null);
      })
  ...
  onRouterOutletActivate(component: any) {
    ...
    if (component instanceof EntrySelectComponent) {
      ...
      if (activeStyle == "appointment")
        this.router.navigate(["/scan-appointment"], { queryParamsHandling:"preserve"});
      else if (activeStyle == "printer") {
        this.router.navigate(["/select-queue"], { queryParamsHandling:"preserve"});
      }
      else if (activeStyle == "groups") {
        this.router.navigate(["/groups"], { queryParamsHandling:"preserve"});
      }
      else if (activeStyle == "ticket") {
        this.router.navigate([{outlets: {print: ["ticket"]}}], { queryParamsHandling:"preserve"});
      }
    }
    else if (activeStyle == "printer" && component instanceof SelectQueueComponent)
      this.showHome = false;
    else if (activeStyle == "appointment" && component instanceof ScanAppointmentComponent)
      this.showHome = false;
