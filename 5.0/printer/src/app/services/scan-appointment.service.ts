...
export class ScanAppointmentService {
  ...
  constructor(private router: Router, private data: DataService, private style: StyleService, private print: TicketService) {}

  async handleScan(data: string): Promise<void> {
    this.state = "";
    await this.router.navigate(["/handle-appointment"], { queryParamsHandling: "preserve"});
