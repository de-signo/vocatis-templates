...
declare global {
  interface Window { 
    CefSharp: { BindObjectAsync(name: string): Promise<{Success: boolean, Message: string}> }|undefined;
    printer: {
...
export class TicketService {
  ...
  constructor(private router: Router, private route: ActivatedRoute, private dataService: DataService, private style: StyleService) {
    if (window.CefSharp) {
      window.CefSharp.BindObjectAsync("printer").then(e => {
        this.isPrinterAvailable = e.Success;
        if (!e.Success) {
          console.warn("No printing extension found. Maybe we're not on a iSign player. " + e.Message);
        }
      });
      window.CefSharp.BindObjectAsync("player").then(e => {
        this.isPlayerAvailable = e.Success;
        if (!e.Success) {
          console.warn("No player extension found. Maybe we're not on a iSign player. " + e.Message);
        }
      });
    }
    else {
      console.log("Not running on an iSign player. Using printer dialog and disable printer status report.");
    }
    
    timer(10000, 60000).subscribe(_ => this.reportPrinterStatus().catch(error =>
      console.error("reportPrinterStatus failed. " + error)
    ));
  }

  private async reportPrinterStatus(): Promise<void> {
    if (!this.isPrinterAvailable || !this.isPlayerAvailable)
      return;
  ...
  async handleGetNewNumber(b: ButtonModel) {
    this.state = "wait";
    this.button = b;
    await this.router.navigate(["/print-status"], { queryParamsHandling: "preserve" });
  
    // get number from server
    let data = await this.dataService.getNewNumber(b.queue, b.categories).pipe(first()).toPromise();
    this.current = data;
    if (this.style.enablePrint) {
      await this.router.navigate([{outlets: {primary: ["print-status"], print: ["ticket"]}}], { queryParamsHandling: "preserve" });
